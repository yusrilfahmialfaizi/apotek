...
function post_data(){
  ...
  if(validator.form()){
    ...
    // $.post('url', data);
    var api = base_url+"admin/produsen/post";
    
    $.post(api, data).done(function( data ) {
      if (data == "1"){
        bootbox.alert("Data produsen berhasil disimpan.", function(){
          location.reload();
        })
      }else{
...
$(document).on('click','.delete',function(){
  ...
  bootbox.confirm("Apakah anda yakin akan menghapus data produsen "+$(tr).find('td').eq(1).html()+" - "+$(tr).find('td').eq(2).html()+"? ", function(result){
    if (result) {
      $.post( base_url+"admin/produsen/delete", {id : idx}).done(function( data ) {
        if (data == '1'){
...
$(document).on('click','.update',function(){
  ...
  var tr = $(this).closest('tr');
  $('#active').show();
  $.post( base_url+"admin/produsen/get_by_id", {idx: idx}).done(function( data1 ) {
    var json = $.parseJSON(data1);
    console.log(json)
    $('[name="nama"]').val(json.nama);
    $('[name="id_produsen"]').val(json.id_produsen);
