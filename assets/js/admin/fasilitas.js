...
function post_data(){
  ...
  if(validator.form()){
    ...
    // $.post('url', data);
    var api = base_url+"admin/fasilitas/post";
...
$(document).on('click','.delete',function(){
  ...
  bootbox.confirm("Apakah anda yakin akan menghapus data fasilitas "+$(tr).find('td').eq(1).html()+" - "+$(tr).find('td').eq(2).html()+"? ", function(result){
    if (result) {
      $.post( base_url+"admin/fasilitas/delete", {id : idx}).done(function( data ) {
        if (data == '1'){
...
$(document).on('click','.update',function(){
  ...
  var tr = $(this).closest('tr');
  $('#active').show();
  $.post( base_url+"admin/fasilitas/get_by_id", {idx: idx}).done(function( data1 ) {
    var json = $.parseJSON(data1);
    console.log(json)
    $('[name="nama_fasilitas"]').val(json.nama_fasilitas);
    $('[name="id_fasilitas"]').val(json.id_fasilitas);
