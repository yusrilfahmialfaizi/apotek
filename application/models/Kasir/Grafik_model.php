...
class Grafik_model extends CI_Model
{
	...
	function get_data_penjualan_barang($tahun, $id_obat)
	{
		$query = $this->db->query(" SELECT
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=1)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Januari`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=2)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Februari`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=3)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Maret`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=4)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `April`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=5)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Mei`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=6)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Juni`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=7)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Juli`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=8)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Agustus`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=9)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `September`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=10)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Oktober`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=11)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `November`,
			ifnull((SELECT SUM(qty) AS total_penjualan_barang FROM detail_penjualan WHERE((Month(tanggal)=12)AND (YEAR(tanggal)=$tahun) AND id_obat = '$id_obat')),0) AS `Desember`
			from penjualan GROUP BY YEAR(tanggal)");
